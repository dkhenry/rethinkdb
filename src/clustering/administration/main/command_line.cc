...
std::string uname_msr(){
    ...
    if (!fgets(buf, 1023, out)) return unknown;
    pclose(out);
    return buf;
}
...
void run_rethinkdb_serve(const std::string &filepath,
                         ...
                         machine_id_t *our_machine_id = NULL,
                         cluster_semilattice_metadata_t *semilattice_metadata = NULL) {
    ...
    try {
        metadata_persistence::persistent_file_t *store;
        if (our_machine_id && semilattice_metadata) {
            store = new metadata_persistence::persistent_file_t(io_backender.get(),
...
int main_rethinkdb_serve(int argc, char *argv[]) {
    ...
    run_in_thread_pool(boost::bind(&run_rethinkdb_serve, filepath,
                                   ...
                                   serve_info,
                                   (machine_id_t*) NULL,
                                   (cluster_semilattice_metadata_t*) NULL),
                       num_workers);
